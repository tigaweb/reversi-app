...
const Board = ({ gameId }) => {
  ...
  return (
    <>
      ...
      <div className='Board
        w-[30rem] m-0 m-auto mt-5 mb-7 grid grid-cols-8 place-content-center place-items-center divide-solid border-0 border-t-[1px] border-l-[1px] bg-gray-300
        ...
        {board.map((line, lineIndex) => {
          return line.map((square, squareIndex) => {
            if (square === 0) {
              return <div key={`${lineIndex}+${squareIndex}`} className="cursor-pointer" onClick={() => dispatch(registerTurn({ game_id: game_id, turn_count: turn_count + 1, disc: next_disc, x: squareIndex, y: lineIndex }))}></div>
            }
            return (
              square === 1
